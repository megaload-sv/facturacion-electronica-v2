...
<div class="container mt-4">
    <h3>🚀 Pruebas Masivas - DTE Sujeto Excluido (14)</h3>
    <form id="formSujetoExcluido" class="row g-3">
<?= csrf_field() ?>
        <div class="col-md-3">
        ...
        <div class="col-md-3 d-flex align-items-end">
            ...
            <button type="button" class="btn btn-danger me-2" id="btnStop" disabled>■ Detener</button>
            <button type="button" class="btn btn-secondary" id="btnExportar">📁 Exportar Resultados</button>
        </div>
    </form>
...
<script>
    ...
    let resultadosExportables = [];

    function ejecutarPruebaSE() {
        ...
        $.getJSON(`<?= base_url('dte/generarSujetoExcluido') ?>`, { inicio: correlativo, total: 1 }, function (response) {
            ...
            <div class="card mb-3">
                <div class="card-header bg-light">
                    <strong>DTE Sujeto Excluido #${actual + 1}</strong> - ${dte.identificacion.numeroControl}
                </div>
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-4">
        ...
        `;
            ...
            resultadosExportables.push({ dte, firma, sello });
    ...
    $('#formSujetoExcluido').on('submit', function (e) {
        e.preventDefault();
        total = parseInt($('#total').val());
        actual = 0;
        correlativo = parseInt($('#inicio').val());
        ejecucionActiva = true;
        resultadosExportables = [];
        ejecutarPruebaSE();
    });
    ...
    $('#btnAuto').on('click', function () {
        ...
        ejecutarPruebaSE();
        intervaloID = setInterval(ejecutarPruebaSE, intervaloSegundos * 1000);
    });

    $('#btnStop').on('click', detenerAutoEjecucion);

    $('#btnExportar').on('click', function () {
        if (resultadosExportables.length === 0) {
            alert('⚠️ No hay resultados para exportar.');
            return;
        }

        const blob = new Blob([JSON.stringify(resultadosExportables, null, 2)], { type: 'application/json' });
        const url = URL.createObjectURL(blob);

        const a = document.createElement('a');
        a.href = url;
        a.download = `dtes_sujeto_excluido_${new Date().toISOString().slice(0, 19).replace(/[:T]/g, '-')}.json`;
